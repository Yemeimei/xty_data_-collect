...
class ZdhSpider(CrawlSpider):
    name = 'ZDH'
    allowed_domains = ['www.ca800.com']
    start_urls = ['http://www.ca800.com/news/autonews/l_1.html']
    custom_settings = {
        # 并发请求
    ...
    rules = (
        Rule(LinkExtractor(restrict_css='.pager-next '), follow=True),
        Rule(LinkExtractor(restrict_css='.newslist_news a'), callback='parse_item', follow=True),
    )
