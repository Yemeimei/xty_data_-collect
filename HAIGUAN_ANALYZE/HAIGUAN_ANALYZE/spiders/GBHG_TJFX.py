...
class GbhgTjfxSpider(scrapy.Spider):
    name = 'GBHG_TJFX'
    # allowed_domains = ['http://gongbei.customs.gov.cn/gongbei_customs/374280/374301/374303/index.html']
    start_urls = ['http://gongbei.customs.gov.cn/gongbei_customs/374280/374301/374303/index.html']
    ...
    #     urls = 'http://39.96.199.128:8888/getCookie?url=http://gongbei.customs.gov.cn/eportal/ui?pageId=434775&currentPage=4&moduleId=6887eda10828436ea29ad29e60cb778a&staticRequest=yes'
    #     yield scrapy.Request(urls, callback=self.parseCookie, meta={
    #         'url': 'http://gongbei.customs.gov.cn/eportal/ui?pageId=434774&currentPage=1&moduleId=1667380986bc42c583c65be8d74da7d1&staticRequest=yes',
    #         'type': 'parse'},
    ...
    def parse(self, response):
        if response.status == 209:
            ...
            url = 'http://gongbei.customs.gov.cn/eportal/ui?pageId=' + page_id + \
    ...
    def parse_total(self, response):
        if response.status == 209:
            ...
            for page_num in range(page_count):
                url = 'http://gongbei.customs.gov.cn/eportal/ui?pageId=' + page_id + '&currentPage=' + \
    ...
    def parse_item(self, response):
        if response.status == 209:
            ...
            try:
                ...
                item['name'] = '中华人民共和国拱北海关'
                item['website'] = '中华人民共和国拱北海关-统计分析'
                item['link'] = response.url
                item['txt'] = ''.join(
                    response.css('#easysiteText *::text').extract())
                item['module_name'] = '中华人民共和国拱北海关-统计分析'
                item['spider_name'] = 'GBHG_TJFX'
                print(
                        "===========================>crawled one item" +
