...
class YbCycSpider(CrawlSpider):
    name = 'YB_cyc'
    allowed_domains = ['www.cpcaauto.com']
    start_urls = ['http://www.cpcaauto.com/news.asp?types=bgzl&Page=1','http://www.cpcaauto.com/news.asp?types=csjd&Page=1s']
    custom_settings = {
        # 并发请求
    ...
    rules = (
        Rule(LinkExtractor(restrict_css=".list_d a "), callback='parse_item', follow=True),
        Rule(LinkExtractor(restrict_css=".Page a "), follow=True),
    )
