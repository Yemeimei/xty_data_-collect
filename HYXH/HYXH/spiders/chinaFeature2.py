...
class HySpider(CrawlSpider):
    name = 'chinaFeature2'
    allowed_domains = ['feature.mei.net.cn']
    start_urls = [
        f'http://feature.mei.net.cn/view/more5.asp?PG={x}'
    ...
    def parse(self, response):
        for href in response.css('.a_hei13::attr(href)').extract():
            print(href)
            yield scrapy.Request(str(href), callback=self.parse_items, dont_filter=True)
    ...
    def parse_items(self, response):
        ...
        item['txt'] = txt
        item['spider_name'] = 'chinaFeature2'
