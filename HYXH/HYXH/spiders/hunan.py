...
class HySpider(CrawlSpider):
    name = 'hunan'
    allowed_domains = ['www.hnyx.org']
    start_urls = [
        f'http://www.hnyx.org/hyxws-1.htm'
        for x in range(1, 2)]
    custom_settings = {
        # 并发请求
    ...
    rules = (
        Rule(LinkExtractor(restrict_css='.news a'), callback='parse_items', follow=True),
    )
    ...
    def parse_items(self, response):
        ...
        item['source'] = '湖南省医疗器械行业协会'
        item['website'] =  '湖南省医疗器械行业协会'
        item['link'] = lyurl
