...
class HySpider(CrawlSpider):
    name = 'ningbo'
    allowed_domains = ['www.nbamdi.com']
    start_urls = [
        f'http://www.nbamdi.com/news/list/77-32-{x}.html'
        for x in range(1, 50)]
    custom_settings = {
        # 并发请求
    ...
    rules = (
        Rule(LinkExtractor(restrict_css='.cont1_left2_list a '), callback='parse_items', follow=True),
    )
    ...
    def parse_items(self, response):
        ...
        item['source'] = '宁波市医疗器械行业协会'
        item['website'] =  '宁波市医疗器械行业协会'
        item['link'] = lyurl
