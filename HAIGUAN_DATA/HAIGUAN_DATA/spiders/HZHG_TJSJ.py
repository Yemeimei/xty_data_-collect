...
class HzhgTjsjSpider(scrapy.Spider):
    name = 'HZHG_TJSJ'
    # allowed_domains = ['http://hangzhou.customs.gov.cn/hangzhou_customs/575609/575611/575612/index.html']
    start_urls = ['http://hangzhou.customs.gov.cn/hangzhou_customs/575609/575611/575612/index.html']
    ...
    #     urls = 'http://39.96.199.128:8888/getCookie?url=http://hangzhou.customs.gov.cn/eportal/ui?pageId=434774&currentPage=1&moduleId=1667380986bc42c583c65be8d74da7d1&staticRequest=yes'
    #     yield scrapy.Request(urls, callback=self.parseCookie, meta={
    #         'url': 'http://hangzhou.customs.gov.cn/eportal/ui?pageId=434774&currentPage=1&moduleId=1667380986bc42c583c65be8d74da7d1&staticRequest=yes',
    #         'type': 'parse'},
    ...
    def parse(self, response):
        if response.status == 209:
            ...
            url = 'http://hangzhou.customs.gov.cn/eportal/ui?pageId=' + page_id + \
    ...
    def parse_total(self, response):
        if response.status == 209:
            ...
            for page_num in range(page_count):
                url = 'http://hangzhou.customs.gov.cn/eportal/ui?pageId=' + page_id + '&currentPage=' + \
    ...
    def parse_item(self, response):
        if response.status == 209:
            ...
            try:
                ...
                item['name'] = '中华人民共和国杭州海关'
                item['website'] = '中华人民共和国杭州海关-统计数据'
                item['link'] = response.url
                item['txt'] = ''.join(
                    response.css('#easysiteText *::text').extract())
                item['module_name'] = '中华人民共和国杭州海关-统计数据'
                item['spider_name'] = 'HZHG_TJSJ'
                print(
                        "===========================>crawled one item" +
