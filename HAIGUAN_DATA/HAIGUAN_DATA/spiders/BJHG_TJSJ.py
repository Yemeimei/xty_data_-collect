...
class BjhgTjsjSpider(scrapy.Spider):
    ...
    #         'url': 'http://beijing.customs.gov.cn/eportal/ui?pageId=434774&currentPage=1&moduleId=1667380986bc42c583c65be8d74da7d1&staticRequest=yes','type':'parse'},
    ...
    def parse(self, response):
        if response.status == 209:
            ...
            yield scrapy.Request(urls, callback=self.parseCookie, meta={'url': str(response.url),'type':'parse'}, dont_filter=True, priority=10)
        else:
    ...
    def parse_total(self, response):
        if response.status == 209:
            urls = 'http://39.96.199.128:8888/getCookie?url=' + str(response.url)
            yield scrapy.Request(urls, callback=self.parseCookie, meta={'url': str(response.url),'type':'parse_total'}, dont_filter=True, priority=10)
        else:
    ...
    def parse_list(self, response):
        if response.status == 209:
            urls = 'http://39.96.199.128:8888/getCookie?url=' + str(response.url)
            yield scrapy.Request(urls, callback=self.parseCookie, meta={'url': str(response.url),'type':'parse_list'}, dont_filter=True, priority=10)
        else:
    ...
    def parse_item(self, response):
        if response.status == 209:
            urls = 'http://39.96.199.128:8888/getCookie?url=' + str(response.url)
            yield scrapy.Request(urls, callback=self.parseCookie, meta={'url': str(response.url),'type':'parse_item'}, dont_filter=True, priority=10)
        else:
            try:
