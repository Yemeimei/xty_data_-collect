...
class KmhgTjsjSpider(scrapy.Spider):
    name = 'KMHG_TJSJ'
    allowed_domains = ['kunming.customs.gov.cn']
    ...
    def parse(self, response):
        if response.status == 209:
            ...
            try:
                yield scrapy.Request('http://kunming.customs.gov.cn/kunming_customs/611308/611314/611316/index.html', callback=self.parse_total, meta=response.meta, dont_filter=True)
            except Exception as e:
        ...
        if response.status == 209:
            ...
            try:
                ...
                for page_num in range(page_count):
                    if page_num < 3:
                        url = 'http://kunming.customs.gov.cn/kunming_customs/611308/611314/611316/86bd4b40-'+str(page_num + 1)+'.html'
                    else:
                        url = 'http://kunming.customs.gov.cn/eportal/ui?pageId=' + page_id + '&currentPage=' + \
                              str(page_num + 1) + '&moduleId=' + module_id + '&staticRequest=yes'

                    print('parse_total url===' + str(url))
